...
const Tahanan = ({ defaultValue }: any) => {
	...
	useEffect(() => {
		...
		const data = async () => {
			try {
				//Total WBP
				const responseWBP = await apiReadAllWBPOtmil(
					token,
					requestData
				);
	...
	const renderPageButtonsSick = () => {
		...
		let startPage =
			currentPageSick <= 3
				? 1
				: Math.floor((currentPageSick - 1) / 3) * 3;
	...
	return (
		<>
			...
			>
				<div className="z-9999 flex h-full justify-end">
					<div className="w-full border bg-white shadow-default dark:border-2 dark:bg-boxdark ">
						<div className="flex ">
							<div className="w-28">
								<h1 className="border-b-2 flex justify-center">
									No Pegawai
								</h1>
							...
							<div className="w-1/4">
								<h1 className="border-x-2 border-b-2 pl-2">
									Nama
								</h1>
							...
							<div className="w-1/4">
								<h1 className="border-l-2 border-b-2 pl-2">
									Status
								</h1>
							...
							<div className="w-1/4">
								<h1 className="border-b-2 border-l-2 pl-2">
									Tanggal Kontrak
								</h1>
			...
			>
				...
				<button
					className={`${
						currentPage === pagenation ? "hidden" : "w-5"
					}`}
					onClick={handleNext}
				>
					<svg
			...
			>
				...
				<button
					className={`${
						currentPageSick === pagenationSick ? "hidden" : "w-5"
					}`}
			...
			>
				...
				<button
					className={`${
						currentPageIsolate === pagenationIsolate
							? "hidden"
							: "w-5"
					}`}
