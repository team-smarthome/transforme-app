...
const ManufactureList = () => {
			...
			steps: [
				...
				},
				{
					...
					popover: {
						...
						description: "Mencari berdasarkan platform",
					},
				},
				{
					element: ".b-search",
				...
				},
				{
					...
					popover: {
						title: "Tambah",
						description: "Menambahkan data manufacture",
					},
				},
			],
		});
	...
	const exportToExcel = async () => {
		const dataToExcel = [
			["Nama Manufacture", "Nama Platform"],
			...data.map((item: any) => [item.manufacture, item.nama_platform]),
		];
		...
		xlsx.writeFile(
			wb,
			`Data-Manufacture ${dayjs(new Date()).format(
				"DD-MM-YYYY HH.mm"
			)}.xlsx`
		);
	};
