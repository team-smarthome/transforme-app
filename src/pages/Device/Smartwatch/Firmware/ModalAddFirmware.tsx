...
import { Alerts } from "./AlertFirmware";
...
export const AddFirmware: React.FC<AddFirmwareModalProps> = ({ closeModal, onSubmit, defaultValue, isDetail, isEdit }) => {
  ...
  const validateForm = () => {
    ...
    for (const [key, value] of Object.entries(formState)) {
      if (key !== "version" && key !== "nama_platform") {
        if (!value) {
          ...
          {isLoading ? (
            ...
            <div className="border-b border-stroke py-4 px-7 dark:border-strokedark">
              <div className="w-full flex justify-between">
                <div>
                  <h3 className="text-xl font-semibold text-black dark:text-white">{isDetail ? "Detail Data Firmware" : isEdit ? "Edit Data Firmware" : "Tambah Data Firmware"}</h3>
                </div>
                ...
                <div className="mt-5 grid grid-cols-2 gap-x-5 gap-y-1 justify-normal">
                  <div className="form-group w-full h-22">
                    ...
                    <input
                      ...
                      disabled={isDetail}
                    />
                    <p className="error-text p-0 m-0">{errors.map((item) => (item === "version" ? "Version Wajib Diisi" : ""))}</p>
                  </div>
                  <div className="form-group w-full h-22">
                    <label htmlFor="firmeware_id" className="capitalize block text-sm font-medium text-black dark:text-white">
                      Platform
                    </label>
                    <Select
                      ...
                      styles={customStyles}
                      name="platform_id"
                      options={platform.map((item: platform) => ({
                        value: item.platform_id,
                        label: item.nama_platform,
                      }))}
                      onChange={handleChangePlatform}
                    />
                    <p className="error-text">{errors.map((item) => (item === "firmeware_id" ? "Pilih Tipe Device" : ""))}</p>
                  </div>
                </div>
