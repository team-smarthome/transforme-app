...
import { AddRegistrationKios } from "./ModalAdd";
import { DeleteRegistrationKiosModal } from "./ModalDelete";
import { Alerts } from "./Alert";
import {
	apiReadRegistrationKios,
	apiCreateRegistrationKios,
	apiUpdateRegistrationKios,
	apiDeleteRegistrationKios,
} from "../../../services/api";
...
interface Item {
	gmac: string;
	nama_registration_kios: string;
	status_registration_kios: string;
...
const RegistrationKiosList = () => {
			...
			steps: [
				{
					...
					popover: {
						title: "Search",
						description: "Mencari nama Registration Kios",
					},
				},
				{
					element: "#p-status",
				...
				},
				{
					...
					popover: {
						title: "Button Search",
						description: "Click button untuk mencari nama Registration Kios",
					},
				},
				{
					element: ".excel",
				...
				},
				{
					...
					popover: {
						title: "Tambah",
						description: "Menambah data registration kios",
					},
				},
			],
		});
	...
	const handleSearchClick = async () => {
		let params = {
			filter: {
				nama_registration_kios: filter,
				status_registration_kios: filterStatus,
		...
		try {
			const response = await apiReadRegistrationKios(params.filter, token);
			setPages(response.data.pagination.totalPages);
			setRows(response.data.pagination.totalRecords);
	...
	const fetchData = async () => {
		...
		setIsLoading(true);
		try {
			const response = await apiReadRegistrationKios(params, token);
			if (response.data.status !== "OK") {
	...
	const handleSubmitDelete = async (params: any) => {
		try {
			const responseDelete = await apiDeleteRegistrationKios(params, token);
	...
	const handleSubmitAdd = async (params: any) => {
		...
		try {
			const responseCreate = await apiCreateRegistrationKios(params, token);
	...
	const handleSubmitEdit = async (params: any) => {
		...
		try {
			const responseEdit = await apiUpdateRegistrationKios(params, token);
			...
			<div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
					...
					<div className="mb-4 flex gap-2 items-center border-[1px] border-slate-800 px-4 py-2 rounded-md">
						<div className="flex w-full">
							<div className="search">
								<SearchInputButton
									value={filter}
									placehorder="Cari nama Registration Kios"
				...
				<div className="flex justify-between items-center mb-3">
					<h4 className="ext-xl font-semibold text-black dark:text-white">
						Data Perangkat Registration Kios
					</h4>
					{!isOperator && (
						<button
							...
							<div>
								...
								>
									...
									<div
										...
										className="cursor-pointer hidden items-center justify-center p-2.5 sm:flex xl:p-5"
									>
										{item.status_registration_kios === "aktif" ? (
										...
										) : item.status_registration_kios === "rusak" ? (
...
export default RegistrationKiosList;
