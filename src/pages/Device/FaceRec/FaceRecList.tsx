...
import { AddFaceRec } from "./ModalAddFaceRec";
import { DeleteFaceRecModal } from "./ModalDeleteFaceRec";
import { Alerts } from "./AlertFaceRec";
import { 
    apiReadFaceRec, 
    apiDeleteFaceRec, 
    apiCreateFaceRec, 
...
interface Item {
  gmac: string;
  nama_face_rec: string;
  status_fec_rec: string;
  jumlah_face_rec: string;
...
const FaceRecList = () => {
  ...
  const handleSearchClick = async () => {
    let params = {
      filter: {
        nama_face_rec: filter,
        status_fec_rec: filterStatus,
    ...
    try {
      const response = await apiReadFaceRec(params, token);
  ...
  const handleSubmitDelete = async (params: any) => {
    try {
      const responseDelete = await apiDeleteFaceRec(params, token);
  ...
  const handleSubmitAdd = async (params: any) => {
    ...
    try {
      const responseCreate = await apiCreateFaceRec(params, token);
  ...
  const handleSubmitEdit = async (params: any) => {
    ...
    try {
      const responseEdit = await apiUpdateFaceRec(params, token);
  ...
  const exportToExcel = async () => {
    const dataToExcel = [
      ...
      ...data.map((item: any) => [
        ...
        item.gmac,
        // item.status_fec_rec,
        item.status_fec_rec === "aktif" ? "tidak aktif" : item.status_fec_rec,
    ...
    <div className="container py-[16px]">
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          ...
          <div className="mb-4 flex gap-2 items-center border-[1px] border-slate-800 px-4 py-2 rounded-md">
            <div className="flex w-full">
              <div className="search">
                <SearchInputButton
                  value={filter}
                  placehorder="Cari nama Face Rec"
        ...
        <div className="flex justify-between items-center mb-3">
          <h4 className="ext-xl font-semibold text-black dark:text-white">Data Perangkat Facerec</h4>
          {!isOperator && (
        ...
        <div className="flex flex-col">
          <div className={`grid  rounded-t-md bg-gray-2 dark:bg-slate-600 ${isOperator ? "grid-cols-5" : "grid-cols-5"}`}>
            <div className="p-2.5 text-center xl:p-5">
              <h5 className="text-sm font-medium uppercase xsm:text-base">Nama Facerec</h5>
            </div>
            <div className="p-2.5 text-center xl:p-5">
          ...
          {modalDetailOpen && <AddFaceRec closeModal={() => setModalDetailOpen(false)} onSubmit={handleSubmitAdd} defaultValue={detailData} isDetail={true} />}
          {modalEditOpen && <AddFaceRec closeModal={handleCloseEditModal} onSubmit={handleSubmitEdit} defaultValue={editData} isEdit={true} />}
          {modalAddOpen && <AddFaceRec closeModal={handleCloseAddModal} onSubmit={handleSubmitAdd} />}
          {modalDeleteOpen && <DeleteFaceRecModal closeModal={handleCloseDeleteModal} onSubmit={handleSubmitDelete} defaultValue={deleteData} />}
        </div>
