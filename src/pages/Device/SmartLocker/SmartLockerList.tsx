...
import { AddSmartLocker } from "./ModalAdd";
import { DeleteSmartLockerModal } from "./ModalDelete";
import { Alerts } from "./Alert";
import {
	apiReadSmartLocker,
	apiCreateSmartLocker,
	apiUpdateSmartLocker,
	apiDeleteSmartLocker,
} from "../../../services/api";
...
interface Item {
	gmac: string;
	nama_smartlocker: string;
	status_smartlocker: string;
...
const SmartLockerList = () => {
			...
			steps: [
				{
					...
					popover: {
						title: "Search",
						description: "Mencari nama Smart Locker",
					},
				},
				{
					element: "#p-status",
				...
				},
				{
					...
					popover: {
						title: "Button Search",
						description: "Click button untuk mencari nama Smart Locker",
					},
				},
				{
					element: ".excel",
				...
				},
				{
					...
					popover: {
						title: "Tambah",
						description: "Menambah data smart locker",
					},
				},
			],
		});
	...
	const handleSearchClick = async () => {
		let params = {
			filter: {
				nama_smartlocker: filter,
				status_smartlocker: filterStatus,
		...
		try {
			const response = await apiReadSmartLocker(params.filter, token);
			setPages(response.data.pagination.totalPages);
			setRows(response.data.pagination.totalRecords);
	...
	const fetchData = async () => {
		...
		setIsLoading(true);
		try {
			const response = await apiReadSmartLocker(params, token);
			if (response.data.status !== "OK") {
	...
	const handleSubmitDelete = async (params: any) => {
		try {
			const responseDelete = await apiDeleteSmartLocker(params, token);
	...
	const handleSubmitAdd = async (params: any) => {
		...
		try {
			const responseCreate = await apiCreateSmartLocker(params, token);
	...
	const handleSubmitEdit = async (params: any) => {
		...
		try {
			const responseEdit = await apiUpdateSmartLocker(params, token);
	...
	const exportToExcel = async () => {
		const dataToExcel = [
			...
			...data.map((item: any) => [
				item.nama_smartlocker,
				item.gmac,
				// item.status_smartlocker,
				item.status_smartlocker === "tidak"
					? "tidak aktif"
					: item.status_smartlocker,
		...
		xlsx.writeFile(
			wb,
			`Data-SmartLocker ${dayjs(new Date()).format("DD-MM-YYYY HH.mm")}.xlsx`
		);
	};
					...
					<div className="mb-4 flex gap-2 items-center border-[1px] border-slate-800 px-4 py-2 rounded-md">
						<div className="flex w-full">
							<div className="search">
								<SearchInputButton
									value={filter}
									placehorder="Cari nama Smart Locker"
...
export default SmartLockerList;
