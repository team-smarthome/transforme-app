...
const GatewayList = () => {
	...
	const handleSearchClick = async () => {
		...
		try {
			const response = await apiReadHelmet(params.filter, token);
			setPages(response.data.pagination.totalPages);
			setRows(response.data.pagination.totalRecords);
	...
	const fetchData = async () => {
		...
		setIsLoading(true);
		try {
			const response = await apiReadHelmet(params, token);
			if (response.data.status !== "OK") {
	...
	const handleSubmitDelete = async (params: any) => {
		try {
			const responseDelete = await apiDeleteHelmet(params, token);
	...
	const handleSubmitAdd = async (params: any) => {
		...
		try {
			const responseCreate = await apiCreateHelmet(params, token);
	...
	const handleSubmitEdit = async (params: any) => {
		...
		try {
			const responseEdit = await apiUpdateHelmet(params, token);
	...
	const exportToExcel = async () => {
		...
		xlsx.writeFile(
			wb,
			`Data-Helmet ${dayjs(new Date()).format("DD-MM-YYYY HH.mm")}.xlsx`
		);
	};
