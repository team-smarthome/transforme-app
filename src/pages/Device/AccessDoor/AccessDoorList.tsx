...
interface Item {
	gmac: string;
	nama_access_door: string;
	status_access_door: string;
	jumlah_access_door: string;
...
const AccessDoorList = () => {
			...
			steps: [
				...
				},
				{
					...
					popover: {
						title: "Button Search",
						description:
							"Click button untuk mencari nama Access Door",
					},
				},
				{
					element: ".excel",
	...
	const handleSearchClick = async () => {
		let params = {
			filter: {
				nama_access_door: filter,
				status_access_door: filterStatus,
		...
		try {
			const response = await apiReadAccessDoor(params.filter, token);
	...
	const fetchData = async () => {
		...
		setIsLoading(true);
		try {
			const response = await apiReadAccessDoor(params, token);
	...
	const handleSubmitDelete = async (params: any) => {
		try {
			const responseDelete = await apiDeleteAccessDoor(params, token);
	...
	const handleSubmitAdd = async (params: any) => {
		...
		try {
			const responseCreate = await apiCreateAccessDoor(params, token);
	...
	const handleSubmitEdit = async (params: any) => {
		try {
			const responseEdit = await apiUpdateAccessDoor(params, token);
	...
	const exportToExcel = async () => {
		...
		xlsx.writeFile(
			wb,
			`Data-Access Door ${dayjs(new Date()).format(
				"DD-MM-YYYY HH.mm"
			)}.xlsx`
		);
	};
