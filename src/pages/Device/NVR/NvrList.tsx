...
const NVRList = () => {
  ...
  const handleSearchClick = async () => {
    ...
    try {
      const response = await apiReadNvr(params.filter, token);
      setPages(response.data.pagination.totalPages);
      setRows(response.data.pagination.totalRecords);
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await apiReadNvr(params, token);
      if (response.data.status !== "OK") {
  ...
  const handleSubmitDelete = async (params: any) => {
    try {
      const responseDelete = await apiDeleteNvr(params, token);
  ...
  const handleSubmitAdd = async (params: any) => {
    ...
    try {
      const responseCreate = await apiCreateNvr(params, token);
  ...
  const handleSubmitEdit = async (params: any) => {
    ...
    try {
      const responseEdit = await apiUpdateNvr(params, token);
  ...
  const exportToExcel = async () => {
    const dataToExcel = [
      ...
      ...data.map((item: any) => [
        item.nama_nvr,
        item.gmac,
        item.status_nvr === "tidak" ? "tidak aktif" : item.status_nvr,
    ...
    xlsx.writeFile(
      wb,
      `Data-nvr ${dayjs(new Date()).format("DD-MM-YYYY HH.mm")}.xlsx`
    );
  };
